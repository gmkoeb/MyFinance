    ...
    >
      {({ setFieldValue, values }) => (
        <Form
          className={
            isMonthly
              ? 'flex flex-col gap-3 edit-bill-form z-10 border px-4 bg-white rounded-lg py-2 absolute'
              : 'flex gap-3 w-full edit-bill-form'
          }
        >
          <div className="flex flex-col">
            <label htmlFor={`billName-${bill_id}`}>Nome</label>
            <Field
              className="rounded p-1 border border-black w-32"
              id={`billName-${bill_id}`}
              name="billName"
          ...
          <div className="flex flex-col">
            <label htmlFor={`billing_company-${bill_id}`}>
              Empresa Cobradora
            </label>
            <Field
              className="rounded p-1 border border-black w-40"
              id={`billing_company-${bill_id}`}
          ...
          {!isMonthly && (
            <>
              ...
              <div className="flex flex-col">
                <label className="mb-2" htmlFor={`paid-${bill_id}`}>
                  Pago
                </label>
                <Field
                  className="h-4"
          ...
          <div className="flex flex-row justify-center items-center gap-3">
            <button
              type="submit"
              className="w-14 rounded-xl h-8 text-center bg-violet-600 mt-5 text-white hover:opacity-80 duration-300"
            >
              Editar
            </button>
            <X
              ...
              width={26}
              color="red"
              className="hover:cursor-pointer mt-5 hover:opacity-60 duration-300"
